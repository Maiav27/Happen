...
const Page17 = () => {
    const FormStore = useStoreState(state => state.pagina17);

    const SetFormStore = useStoreActions(state => state.setForm);
    const [pag17, setPag17] = useState(FormStore)
  
    function salvarRadio(){
      FormStore.pagina17  = pag17;
 ...
 return(
    ...
    <input id="question-2-answer-a" type="radio" name="favelang" checked={pag17 === 1}  onChange={()=>setPag17(1)}/> Concordo totalmente<br/>
            ...
            <div id="submit2" class="button"> <div id="submit1" class="button"><Link style={{ textDecoration : 'none', color : 'black'  }} onClick={salvarRadio} to ={'/Page18'} >Próximo</Link></div></div>
