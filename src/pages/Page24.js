...
const Page24 = () => {
    const FormStore = useStoreState(state => state.pagina24);

    const SetFormStore = useStoreActions(state => state.setForm);
    const [pag24, setPag24] = useState(FormStore)
  
    function salvarRadio(){
      FormStore.pagina24  = pag24;
 ...
 return(
    ...
    <input id="question-2-answer-a" type="radio" name="favelang" checked={pag24 === 1}  onChange={()=>setPag24(1)}/> Concordo totalmente<br/>
            ...
            <div id="submit2" class="button"> <div id="submit1" class="button"><Link style={{ textDecoration : 'none', color : 'black'  }} onClick={salvarRadio} to ={'/Page25'} >Próximo</Link></div></div>
